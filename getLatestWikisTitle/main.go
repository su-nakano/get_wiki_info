...
func topUpSlackInformation(consumedPoints string, prevSprint string, nextSprint string) string {
	thisThursday := time.Now().AddDate(0, 0, 4).Format("01/02")
	nextWednesday := time.Now().AddDate(0, 0, 3).Format("01/02")
...
func main() {
	...
	currentSprintNum := "082"
	prevSprint := fmt.Sprintf("sp-%s", currentSprintNum)
	nextSprintNum, nextSprintNumErr := strconv.Atoi(currentSprintNum)
	...
	if len(items) > 0 {
		...
		sortedStrings := append(wikiUrl, releaseLines...)
		...
		staticText := []string{
			notifyMember,
			"",
			topUpSlackInformation("12", prevSprint, nextSprint),
		}
